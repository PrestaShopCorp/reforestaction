				...
				{else}
					...
					{block name="input"}
						...
						{elseif $field['type'] == 'textarea'}
							<textarea name={$key} cols="{$field['cols']}" rows="{$field['rows']}" {if isset($field['disabled']) && $field['disabled']}disabled="disabled"{/if}>{$field['value']|escape:'htmlall':'UTF-8'}</textarea>
